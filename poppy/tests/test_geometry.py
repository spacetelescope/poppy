...
def test_area_minimal_circle():
    ...
    assert _np.abs(res.sum() -np.pi) < 1e-7
...
def test_pixwt():
    ...
    def check_pixwt_result( args, expected=1, tolerance=1e-5):
        assert abs(geometry.pixwt(*args)-expected).max() < tolerance
    ...
    xa = _np.asarray([0,0,1,1,2,-1])
    ya = _np.asarray([0,1,1,0,0,-1])
    exp = _np.asarray([ 1.0000000, 0.45661139, 0.078786805, 0.45661139, 8.9406967e-08, 0.078786790 ])
