...
class PhysicalFresnelWavefront(FresnelWavefront):
    ...
    def sigma2(self, mask=1.0):
        ...
        dx = self.dx
        intensity = self.intensity*mask
        sigma_xx = dx**2 * np.sum(np.dot((x-center_x)**2, intensity)) / P
        sigma_yy = dx**2 * np.sum(np.dot(intensity, (x-center_y)**2)) / P
        sigma_xy = dx**2 * np.dot(np.dot((x-center_y), intensity), (x-center_y)) / P
    ...
    def M2(self, direction='xy'):
        ...
        Parameters
        ----------
        wf : PhysicalFresnelWavefront object
            Wavefront to compute the beam quality factor for.
        direction : string
